document.addEventListener("DOMContentLoaded", async () => {
    ...
    saveProductBtn.addEventListener("click", async () => {
        ...
        try {
            const response = await fetch("http://192.168.77.39:3000/productos", {
                method: "POST",
                headers: {
            ...
            productCard.innerHTML = `
                <img src="http://192.168.77.39:3000${producto.imagenProducto}" alt="${producto.nombre}" />
    ...
    try {
        ...
        const response = await fetch(`http://192.168.77.39:3000/productos?id=${emprendimientoId}`, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
