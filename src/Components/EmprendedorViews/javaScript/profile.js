document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadBusinessData() {
        try {
            const token = localStorage.getItem("authToken");
            const response = await fetch("http://localhost:3000/emprendimientos/perfil", {
                headers: {
            ...
            profileImageNav.src = `http://localhost:3000${emprendimientoData.imagenEmprendimiento}`;
    ...
    async function saveBusinessSettings() {
        ...
        try {
            const response = await fetch("http://localhost:3000/emprendimientos/actualizar", {
                method: "PUT",
                headers: {
