document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        const emprendimientoResponse = await fetch(`http://192.68.134.131/emprendimientos/${emprendimientoId}`);
        ...
        const productosResponse = await fetch(`http://192.68.134.131/productos?id=${emprendimientoId}`, {
            headers: {
        ...
        if (Array.isArray(productos) && productos.length > 0) {
            productos.forEach(producto => {
                productosLista.innerHTML += `
                    <div class="producto">
                        <img src="http://192.68.134.131${producto.imagenProducto || '/uploads/defaultImage.png'}" 
